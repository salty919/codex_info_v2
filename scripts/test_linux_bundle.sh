...
other_listener_pid=4343
mkdir -p -- "$fake_proc/$other_listener_pid/fd"
ln -s -- 'socket:[9002]' "$fake_proc/$other_listener_pid/fd/3"
printf '  sl local_address rem_address st tx_queue tr tm->when retrnsmt uid timeout inode\n0: 0100007F:2253 00000000:0000 0A 00000000:00000000 00:00000000 00000000 1000 0 9002 1\n' > "$fake_proc/net/tcp"
if HOME="$fake_home" CODEX_HOME="$fake_home/.codex" PATH="$fake_bin:$ORIGINAL_PATH" FAKE_LOG="$log" \
    FAKE_MAIN_ENABLED=1 FAKE_MAIN_ACTIVE=1 FAKE_MAIN_PID="$health_pid" \
    FAKE_HEALTH_VERSION="$health_version" CODEX_INFO_PROC_ROOT="$fake_proc" \
    SYSTEMCTL_BIN=systemctl CURL_BIN=curl \
    bash "$fake_home/.local/libexec/codex-info-install.sh" --verify-runtime >/dev/null 2>&1; then
    fail 'listener owned by a different PID unexpectedly passed runtime verification'
fi
rm -r -- "$fake_proc/$other_listener_pid"
printf '  sl local_address rem_address st tx_queue tr tm->when retrnsmt uid timeout inode\n0: 0100007F:2253 00000000:0000 0A 00000000:00000000 00:00000000 00000000 1000 0 9002 1\n' > "$fake_proc/net/tcp"
if HOME="$fake_home" CODEX_HOME="$fake_home/.codex" PATH="$fake_bin:$ORIGINAL_PATH" FAKE_LOG="$log" \
    FAKE_MAIN_ENABLED=1 FAKE_MAIN_ACTIVE=1 FAKE_MAIN_PID="$health_pid" \
    FAKE_HEALTH_VERSION="$health_version" CODEX_INFO_PROC_ROOT="$fake_proc" \
    SYSTEMCTL_BIN=systemctl CURL_BIN=curl \
    bash "$fake_home/.local/libexec/codex-info-install.sh" --verify-runtime >/dev/null 2>&1; then
    fail 'listener without an accessible owner unexpectedly passed runtime verification'
fi
mkdir -p -- "$fake_proc/$other_listener_pid/fd"
ln -s -- 'socket:[9001]' "$fake_proc/$other_listener_pid/fd/3"
printf '  sl local_address rem_address st tx_queue tr tm->when retrnsmt uid timeout inode\n0: 0100007F:2253 00000000:0000 0A 00000000:00000000 00:00000000 00000000 1000 0 9001 1\n' > "$fake_proc/net/tcp"
if HOME="$fake_home" CODEX_HOME="$fake_home/.codex" PATH="$fake_bin:$ORIGINAL_PATH" FAKE_LOG="$log" \
    FAKE_MAIN_ENABLED=1 FAKE_MAIN_ACTIVE=1 FAKE_MAIN_PID="$health_pid" \
    FAKE_HEALTH_VERSION="$health_version" CODEX_INFO_PROC_ROOT="$fake_proc" \
    SYSTEMCTL_BIN=systemctl CURL_BIN=curl \
    bash "$fake_home/.local/libexec/codex-info-install.sh" --verify-runtime >/dev/null 2>&1; then
    fail 'listener shared by multiple PIDs unexpectedly passed runtime verification'
fi
rm -r -- "$fake_proc/$other_listener_pid"
printf 'case listener PID ownership rejection: PASS\n'
