...
| CUM-138-06 | UX | `src/main.rs` graph projection、Windows Graphing/ViewModel/Control | UX正本の有限表示状態について開始・中間・終端値、線種、period ID、右端ラベルを直接検証し、ASTRA指定4単価、既知/incomplete破線、period選択後pollのbounded reset aliasを同じ到達経路へ統合 | implemented |
