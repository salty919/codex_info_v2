...
| WF-FEAT-SELECTIVE-01 | PRODUCT | `.github/workflows/feat-integration.yml`、`.github/workflows/selective-quality.yml`、分類器 | AGENTS authority-only、history-graph、28-path内の実差分だけを選ぶmodel-history、REST-172の3 pathとRust直接7 caseだけを選ぶresident-publication、U128-19の5 pathとRust直接4 caseだけを選ぶrecorder-gap、I182の3 pathとRust直接12 caseだけを選ぶapp-server-isolation、workflow-selection、profile欠落/重複/未知/所有外path、rename/copy、release非縮小の有限causal caseと実PR job graph | implemented |
...
| U128-37 | PRODUCT | `packaging/install_linux_bundle.sh`、`scripts/test_linux_bundle.sh`、`docs/PRODUCT_REQUIREMENTS.md`、`docs/REQUIREMENTS_LEDGER.md`、`scripts/ci_change_scope.py`、`scripts/quality_plan.py`、`scripts/pre_pr_gate.sh`、`scripts/test_ci_change_scope.py`、`scripts/test_quality_plan.py` | 1) valid emergency→candidate / old generation未起動; 2) candidate readiness fail→byte-identical emergency復元+fresh recorder; 3) identity mismatch→stop前`SAFE_BLOCKED`; 4) `current_switched`/`activation_requested`/`candidate_verified` interruption resume; 5) `rollback_switched` restart/resume; 6) fallback後next update/no-update | implemented |
