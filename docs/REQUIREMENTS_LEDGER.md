...
| CUM-138-06 | UX | `src/main.rs` graph projection、Windows Graphing/ViewModel/Control | 2026-09-03実障害形状（$176→$87台→$184、85→81%）、open local gap、unattributed quota、confirmed gap固定caseをX/Windowsで独立検証 | implemented |
...
| WIN-PARITY-CURRENT-01 | UX | Rust period projection、Windows `EffectiveGraphEnd` | X/Windows同一fixtureのcurrent/過去右端exact値 | verified |
| WIN-PARITY-FIRSTOBS-01 | UX | 共通graph fixture、Rust/Windows projectionとrender adapter | 独立固定期待値、no-diagonal pixel/geometry検査、同revision実Windows Graph画像 | verified |
...
| X-GRAPH-01 | UX | Linux details client、`src/main.rs` graph projection | delayed/rollover fixed fixture tests、X11 graph visual gate | implemented |
| X-GRAPH-02 | UX | service HistoryCanonicalizer、Linux details client、graph selection/projection | rollover A→B、boundary bucket、全graph unit、同一release revisionのfresh X実画面と周期refresh後再確認 | implemented |
...
| WIN-GRAPH-01 | UX | Windows details client、`windows-client/src/CodexInfo.WindowsClient/Graphing` | delayed/rollover graph projection tests、実Windows過去期間画像 | implemented |
...
| WF-FEAT-SELECTIVE-01 | PRODUCT | `.github/workflows/feat-integration.yml`、`.github/workflows/selective-quality.yml`、分類器 | 各owner・共有path・未知pathの有限代表例、異常差分、実PR job graph、ruleset/protection read-back | implemented |
