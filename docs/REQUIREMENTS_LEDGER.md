...
| CUM-138-06 | UX | `src/main.rs` graph projection、Windows Graphing/ViewModel/Control | UX正本の有限表示状態について開始・中間・終端値、利用増加の太い実線・未使用の細い実線・欠測予測の細く密な破線、token正本＋raw Remainingのidle必要十分条件、限定1 cadence bridge、gridを透過しない連続band、active秒だけへ配分するRemaining平滑化、period ID、右端ラベルを両platformで直接検証 | implemented |
| G137-GRAPH-01 | PRODUCT | `src/main.rs` graph projection、Windows Core/ViewModels/Graphing、保存fixture/E2E | `G137-1`..`G137-10`の同一literal inputについて両platformがmodel universe、monotonic accepted値、measured solid／prediction dashedの連続性、垂直segment 0件、表示metric非依存の全model token＋raw Remaining idle、誤検知反例、限定1 cadence bridge、隣接band結合、active秒quota平滑化、最新label、pair/cursor回復、failure isolationを独立照合 | implemented |
...
| REST-172 | PRODUCT | `src/main.rs`、`src/server.rs`、`docs/PRODUCT_REQUIREMENTS.md` | no-op owner tickと同一canonical Thread完了でDB/Session/history/serialization 0、split requestでlegacy全履歴serialize 0、60秒recorder ack 2回前進 | implemented |
...
| WF-NONBLOCKING-QUALITY-01 | PRODUCT | `.github/workflows/version-prepare.yml`、`.github/workflows/feat-integration.yml`、repository rules | actionlint、生成H1の単一status pending→success/failure、H2/non-binary/featのstatus重複0、local acceptance成功/異常、remote非合格表示とruleset非block | implemented |
...
| SEC-CI-TRUST-01 | SECURITY | version・release workflow、local trust gate | actionlint、untrusted checkout検査、non-fast-forward fixture、trusted生成H1だけへの固定context status mutation | implemented |
