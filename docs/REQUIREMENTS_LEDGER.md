...
| WF-FEAT-SELECTIVE-01 | PRODUCT | `.github/workflows/feat-integration.yml`、`.github/workflows/selective-quality.yml`、分類器 | AGENTS authority-only、history-graph、28-path内の実差分だけを選ぶmodel-history、REST-172の3 pathとRust直接6 caseだけを選ぶresident-publication、U128-19の5 pathとRust直接4 caseだけを選ぶrecorder-gap、workflow-selection、profile欠落/重複/未知/所有外path、rename/copy、release非縮小の有限causal caseと実PR job graph | implemented |
