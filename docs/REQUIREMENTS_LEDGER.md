...
| REST-172 | PRODUCT | `src/main.rs`、`docs/PRODUCT_REQUIREMENTS.md` | `unchanged_resident_tick_reuses_snapshot_and_worker_event_publishes_once` | implemented |
...
| WF-BINARY-IMPACT-01 | PRODUCT | `AGENTS.md`、`scripts/ci_change_scope.py`、`scripts/test_ci_change_scope.py`、`.github/workflows/*` | tracked全path、add/delete、rename両端、copy追加先、unknown/空差分、actionlint | implemented |
| WF-FEAT-SELECTIVE-01 | PRODUCT | `.github/workflows/feat-integration.yml`、`.github/workflows/selective-quality.yml`、分類器 | PR本文・機能別profile・exact機能path表なしで完全差分を安定ownerへ分類し、影響ownerの通常品質だけを各1回実行する。空差分、未知責務、malformed rename/copy、release非縮小の有限causal caseと実PR job graph | implemented |
| WF-QUALITY-ONCE-01 | PRODUCT | `.github/workflows/*`、`scripts/resolve_pr_quality.py`、各直接test | 完全diffのowner選択、変更PowerShellだけの構文、draft skip、stale cancel、feat/mainのrelease境界、選択jobのActions結論を直接検証 | implemented |
| WF-NONBLOCKING-QUALITY-01 | PRODUCT | `.github/workflows/main-quality.yml`、`.github/workflows/feat-integration.yml`、repository rules | actionlint、local acceptance成功/異常、custom check mutation 0、remote ruleset/protection/check-runs | implemented |
| WF-POSTMERGE-01 | PRODUCT | `.github/workflows/release.yml`、`scripts/release_authority.py`、`scripts/publish_release.py`、各直接test | 両event順序、空の`workflow_run.pull_requests`を含む構造化PR/final-head対応、最新run attempt 1、旧成功fallback禁止、version input blobによるbinary/non-binary判定、両platform pair、0件/欠落/重複、Draft transactionを検証 | implemented |
| VER-FORWARD-01 | PRODUCT | `.github/workflows/main-quality.yml`、`scripts/resolve_pr_quality.py`、`scripts/product_version.py`、版数3ファイル | main binaryは3ファイル同値かつbaseより単調増加、non-binaryは同値をread-only検証し、patch/minor/major fixtureとActions branch mutation 0を確認 | implemented |
| WF-SERIAL-01 | PRODUCT | `.github/workflows/main-quality.yml`、`.github/workflows/release.yml`、release scripts | PRごとのstale run cancel、同tag二信号の直列化、orphan/Draft/partial/mismatch拒否 | implemented |
...
| SEC-RELEASE-CODEQL-01 | SECURITY | `.github/workflows/codeql.yml`、`.github/workflows/selective-quality.yml`、`scripts/ci_change_scope.py` | 変更された解析対象source→actions/csharp/python/rustの有限代表例、非対象sourceの言語非選択、実CodeQL analysisとalert upload | implemented |
| LINUX-BUNDLE-TARGET-01 | PRODUCT | `scripts/build_linux_bundle.sh`、`scripts/test_linux_bundle.sh`、`.github/workflows/linux-release-quality.yml`、`docs/PRODUCT_REQUIREMENTS.md` | targetとglibc minimumの固定manifest、欠落・不一致のfail-closed fixture | implemented |
| LINUX-BUNDLE-RELEASE-01 | PRODUCT | `.github/workflows/linux-release-quality.yml`、`.github/workflows/selective-quality.yml`、release scripts、直接test、`docs/PRODUCT_REQUIREMENTS.md` | main binaryのLinux build 1回、両platform candidate identity、Linux-only差分のWindows追加選択、片platform欠落拒否 | implemented |
