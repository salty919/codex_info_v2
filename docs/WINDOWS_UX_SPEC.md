...
- model系列の有限表示状態は次を正本とし、model名ごとの全直積には展開しない。
  ...
  | 入力状態 | 表示契約 |
  | --- | --- |
  | 同一periodの確定累積が増加／不変 | 増加は通常線、不変は細い通常線。最初と最後の値を同じ系列へ使う |
  | 当該model値は既知、全model集合は不完全 | 既知の開始値・中間値・終端値を破線で保持し、値そのものを捨てない |
  | 前後の既知点間だけが未観測 | 前後点を破線で結び、途中の消費時刻・速度を確定表示しない |
  | model値自体が未取得、または確認済みrecorder停止区間 | 値を0や直前値にせず切断し、欠損帯を表示する |
