...
| WIN-UPDATE-02 | A supported installed client must converge to a newer trusted stable release without requiring a click or a healthy Linux backend. This supersedes the former check-and-notify-only contract for the first fixed candidate and later. | Normal startup begins the shared update state machine independently of connection/auth/details state. Per-user non-admin logon and hourly tasks run the same installed executable with `--update-only`, create no Window, and finish with a bounded exit code. Existing 1.0.25 cannot acquire this behavior by itself and therefore requires one explicit verified-Setup bootstrap; that migration exception is not the steady-state contract. | Startup/backend-failure tests, task/action inspection, no-HWND update-only process, installed previous-fixed-version to candidate E2E | requirements fixed; product and same-SHA installed evidence pending |
...
| WIN-UPDATE-04 | Concurrent or interrupted update triggers must not launch overlapping Setup processes or report a partial update as success. | UI/startup/logon/hourly/retry share one resolver, coordinator, zero-wait exclusive lease, and atomic pending owner. Pending records attempt/target/source/hash/path/time before launch and exact PID/start/path after launch. Target installed read-back is the only success; exact Setup alive is busy; exited with old version is failed; alive beyond ten minutes is SAFE_BLOCKED and is not killed or overlapped. Pending-write failure launches nothing; PID-persist failure blocks another launch. Discovery is bounded to 30 seconds, download to five minutes, trigger to ten minutes, and same-trigger retry count is zero. | Coordinator concurrency/crash/hang/pending tests, two-trigger process-count E2E, installed generation read-back | requirements fixed; product and same-SHA installed evidence pending |
| WIN-UPDATE-05 | A Windows candidate must prove an actual previous-public-stable to exact-candidate transition before publication. | On fresh non-admin Windows, install the exact public previous stable, invoke its product update route, and externally read back version transition, candidate source/digest, payload, shortcut, Apps registration, logon/hourly tasks, settings hash preservation, process, real UI, and same-source Linux readiness. Same-version reinstall, clean install, unit/mock evidence, asset existence, and post-publication checks are not substitutes. FAIL, INCONCLUSIVE, unexecuted, or environment gap means `publish=false`. | Trusted pre-publication Windows transition job joined with same-source Linux readiness | requirements fixed; workflow/product/live evidence pending |
...
| WIN-PAR-06 | Preserve graph and history. | Current/old periods, dollar/token metric, model visibility, no-history state, persistence. Period selection immediately acknowledges input, retains the prior complete graph on failure/cancel, and projects only the accepted details root without DB access or sample merge/recalculation. The existing presentation-only delayed-quota interpolation and terminal hold remain allowed for confirmed-gap-free active intervals and are never written back. Right-edge labels remain at their series endpoints, move only to resolve collisions, preserve vertical order, and have zero crossing leader lines as in the X graph. X's 1%/99% vertical safety area, dedicated endpoint-label gutter, 1px flat/3px rising model paths, first-observation boundary, and sub-pixel idle-band suppression are mandatory rendering invariants. | Strict details history and graph samples | Graph fixture/cancellation/label-layout tests, resident SQLite persistence tests, fresh current/loading Graph images at 700x480, 940x640, and 1200x800, physical click P90/P95, independent parity review | open |
...
| REG-05 | A revision touching account, thread, local usage, history, graph, or timer paths must confirm the shared rollover fixture, `graph_delayed_quota`, and existing history/rolling/delayed/gap/no-history/REST/Windows regressions against that same revision before completion. | Integrate these finite causal cases into existing test commands; do not add a new workflow gate or all-product matrix. Any non-zero test-count failure, missing case, stale revision, last-good change on reject, or X/Windows oracle mismatch keeps the revision open. | open |
