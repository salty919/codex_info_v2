...
| WIN-UPDATE-01 | A binary-impact version increase merged into `main` must produce the combined Windows/Linux release through the normal GitHub flow. | `Directory.Build.props` is one of the three synchronized stable `X.Y.Z` authorities. PRs validate but cannot publish. A successful main-quality exact head may create `windows-vX.Y.Z` only after the Windows and Linux candidates agree. Creation uploads exact assets to a Draft before publication; an already-published exact state is a no-op, while orphan, Draft, partial, mismatched, unchanged/decreased/invalid, network, and 5xx states never publish or clobber. | `WF-POSTMERGE-01` and `VER-FORWARD-01` direct fixtures, Release asset inspection | in-progress; remote candidate/publication evidence pending |
