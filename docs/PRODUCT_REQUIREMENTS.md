...
7. Windows版の初回起動では、health readiness後に最初のstrict validation済み`/v3/current` generation（新resourceがexact 404の旧serviceだけ上記legacy chain）が揃うまで内容領域を表示せず、固定レイアウト上にスピナーを表示する。control応答とのmerge、途中fieldの順番描画をせず、初回取得失敗時はスピナーを解除して失敗状態と再試行手段を表示する。GraphまたはThreadsを開いていない間は、そのresourceを取得しない。
